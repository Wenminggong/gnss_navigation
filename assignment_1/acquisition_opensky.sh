...
for n in {1..32}; do
    ...
    # 2. 运行 GNSS-SDR 实验
    gnss-sdr --config_file=./assignment_1_gnss_sdr_opensky.conf --log_dir=./log/opensky
    ...
    find . -type f -name "acquisition_opensky_G_1C_ch_0_*_sat_${n}.mat" ! -name "acquisition_opensky_G_1C_ch_0_1_sat_${n}.mat" -delete
    cd ..  # 返回上级目录
    cd ..
    cd ..
done
