...
def com_plot_position_results(file_path_1:str, file_path_2:str, data_type:str):
    result_path_1 = file_path_1
    result_path_2 = file_path_2
    assert os.path.exists(result_path_1) and os.path.exists(result_path_2), "file pathes not existing!"
                
    with h5py.File(result_path_1, 'r') as f:
        latitude_1 = np.array(f["latitude"])
        longitude_1 = np.array(f["longitude"])
    with h5py.File(result_path_2, 'r') as f:
    ...
    if data_type == "opensky":
        ...
        name_1 = "Previous Setting"
        name_2 = "Improved Setting"
    ...
    scatter = ax1.scatter(longitude_sampled_1, latitude_sampled_1, 
                        c='blue', s=30, alpha=0.8, label='Estimated Positions from {}'.format(name_1))
    scatter = ax1.scatter(longitude_sampled_2, latitude_sampled_2, 
                        c='orange', s=30, alpha=0.8, label='Estimated Positions from {}'.format(name_2))
    ...
    ax1.set_title("Position Estimation Distribution between {} and {}".format(name_1, name_2), fontsize=12)
    ...
    if data_type == "opensky":
        plt.savefig(os.path.join("assignment_2/results/task_3/", "com_{}_results.png".format(data_type)))
    elif data_type == "urban":
        plt.savefig(os.path.join("assignment_2/results/task_2/", "com_{}_results.png".format(data_type)))
    plt.show()


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--file_path_1", type=str, default="assignment_1/results/wls/urban/pvt.mat", help="file path 1.")
    parser.add_argument("--file_path_2", type=str, default="assignment_2/results/task_2/pvt.mat", help="file path 2.")
    parser.add_argument("--data_type", type=str, default="urban", help="data type.")
    args = parser.parse_args()

    com_plot_position_results(args.file_path_1, args.file_path_2, args.data_type)